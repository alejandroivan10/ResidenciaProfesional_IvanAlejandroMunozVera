...
%Configuraciones de conexión UDP
u = udp('192.168.1.126',63604);   %  , 'LocalPort', 8887
fopen(u);
...
% Ac_Adj = [9.819   9.584   10.371   0.150   0.131   0.217]; %Acel_Calib 2
% Gy_Adj = [-1.6558    2.0798   -0.3839]; % Gyro_Calib 2
% Mg_Adj = [39.6496   43.5980   40.8667   -22.9593   22.0987   20.4591]; %Mag_Calib 2

% Ac_Adj = [10.132   9.990   10.441   0.236   0.054   -0.713]; %Acel_Calib 3
% Gy_Adj = [3.5523    0.6303    2.2918]; % Gyro_Calib 3
% Mg_Adj = [45.687   44.505   46.507   12.771   14.005   0.820]; %Mag_Calib 3

% Ac_Adj = [9.624   9.603   9.934   0.046   -0.138   -0.480]; %Acel_Calib 4
% Gy_Adj = [1.2490   -1.9366   -1.2892]; % Gyro_Calib 4
% Mg_Adj = [40.687   40.505   40.507   0   15   8]; %Mag_Calib 4

Ac_Adj = [9.886   10.044   10.105   0.039   0.013   -0.249]; %Acel_Calib 5
Gy_Adj = [-0.9449   -2.3539    1.9637]; % Gyro_Calib 5
Mg_Adj = [40.356   36.757   42.856   -40.802   27.426   -5.306]; %Mag_Calib 5
...
%                         Mg_Adj_Store(i_adj,:) = Mg_Adj;
    ...
    SEeuler = RotMat2Euler(SErotM');
    SEeuler = (SEeuler < 0).*360 + SEeuler;
    SEeuler(2:3) = 360 - SEeuler(2:3);
    datSEND =num2str(SEeuler,'%f,%f,%f');
    fwrite(u, datSEND); 
    fprintf('%s\n',datSEND);
