...
/*
 * INDISPENSABLES:
 *  + DEFINIR UN PROTOCOLO DE COMUNICACIÓN "SPI" O "I2C"
*/
//Seleccionar protocolo: #define I2C_PROTOCOL o bien SPI_PROTOCOL
#define I2C_PROTOCOL

//Seleccionar MARG sensor: #define BMX055 o bien MPU9250
#define MPU9250
...
#include <SPI.h>
  ...
  #define MAGread(Addr,CS,Reg,nBytes,Data) SPIreadMg(CS,Reg,nBytes,Data)
  #define MAGwriteByte(Addr,CS,Reg,Data) SPIwriteByteMg(CS,Reg,Data)
  #define MAGwriteNBytes(Addr,CS,Reg,nBytes,Data) SPIwriteNBytesMg(CS,Reg,nBytes,Data)
#endif
...
void MARG_Reset(){
  ...
  MARGwriteByte(MAG_ADDRESS, CS, MAG_CNTL_1+1, 0x01);   //Soft Reset Magnetometer Registers
}
void MARG_setUp(){
  ...
  MARGwriteByte(MAG_ADDRESS, CS, MAG_CNTL_1, MAG_MODE_2 | MAG_FSCALE_16_bit);  
...
void setup()
{
  ...
  //   MARGwriteByte(MAG_ADDRESS, CS, MAG_CNTL_1, 0x00);  //Set -> Power Down Mode
  //   delay(10);
  //   MARGwriteByte(MAG_ADDRESS, CS, MAG_CNTL_1, 0x0F);  //Set -> Fuse ROM access Mode
  //   delay(10);
  //   MARGread(MAG_ADDRESS, CS, MAG_ASAX, 3, ASA);  //Get sensivility
...

  {
    ...
    //   MARGwriteByte(MAG_ADDRESS, CS, MAG_CNTL_1, 0x10);  //Set -> Power Down Mode
    //   MARGwriteByte(MAG_ADDRESS, CS, MAG_ASTC, 0x40); //Magnetic Fiel for SelfTest starts
    //   delay(10);
    //   MARGwriteByte(MAG_ADDRESS, CS, MAG_CNTL_1, 0x18);  //Set -> SELF-TEST Mode
    //   do{
    //      MARGread(MAG_ADDRESS, CS, MAG_STATUS_1, 1, &ST1);
    //   } while (!(ST1 & 0x01));
    //   MARGread(MAG_ADDRESS, CS, MAG_DATA, 7, Mag);
    //   MARGwriteByte(MAG_ADDRESS, CS, MAG_ASTC, 0x00);
    //   MARGwriteByte(MAG_ADDRESS, CS, MAG_CNTL_1, 0x10);  //Set -> Power Down Mode
    //   delay(50);
...
//Indispensable para usar "SPI" sobre el Magnétometro MPU9250
  ...
  //     MARGread(MAG_ADDRESS, CS, Reg, 1, &Data);
